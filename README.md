# Mujagi77 | Logic Hub 🧠

Dobrodošli u **Logic Hub** – digitalnu destinaciju za logičke i matematičke izazove. Ovaj projekat je kreiran sa ciljem da testira vašu logiku, koncentraciju i strpljenje kroz seriju interaktivnih igara.

## 🚀 O projektu
Logic Hub je skup laganih, brzih i optimizovanih igara koje rade direktno u vašem pretraživaču. Nema instalacije, nema reklama – samo čista logika.

## 🎮 Igre koje možete igrati
* **Logička Vaga** – Pronađite različitu lopticu na vagi uz minimalan broj merenja.
* **Lavirint** – Pronađite pravi put kroz lavirint i testirajte svoju snalažljivost.
* **Magična Karta** – Zabavan matematičko-magijski trik u kome igra uvek pogađa kartu koju ste zamislili.
* **Slagalica** – Slaganje i pomeranje brojeva u tačan redosled.
* **Neonski Odboj** – Brzi test refleksa, spretnosti i preciznosti u neon stilu.
* **Izazov 100** – Interaktivni kviz i test znanja sa različitim zadacima.
* **Kraljeva Riznica** – Rešite klasičan matematički problem: otkrijte lažnu vreću zlata sa samo jednim merenjem na vagi.
* **Okreni Obrni 3D** – Rotiranje kocke, slaganje svake strane u istoj boji i otključavanje novih tema.

## 🌐 Posetite sajt
Isprobajte igre odmah: 👉 **[https://mujagi77.github.io/](https://mujagi77.github.io/)**

## ✨ Ključne karakteristike
*   **Minimalistički dizajn:** Fokusiran na korisničko iskustvo (Dark mode / Neon stil).
*   **Brzina:** Optimizovan kod bez nepotrebnih skripti.
*   **Responsivnost:** Igre savršeno rade na telefonima, tabletima i kompjuterima.
*   **Audio povratne informacije:** Zvučni efekti za bolji gejming doživljaj.

## 🛠 Tehnologije
Projekat je razvijen koristeći standardne web tehnologije:
*   HTML5
*   CSS3 (sa fokusom na performanse i animacije)
*   JavaScript (Vanilla - bez eksternih biblioteka)

## 👤 Autor
**Miljan Rančić (mujagi77)**
*   YouTube: [https://www.youtube.com/@mujagi77](https://www.youtube.com/@mujagi77)

---
© 2026 Mujagi77. Sva prava zadržana.
